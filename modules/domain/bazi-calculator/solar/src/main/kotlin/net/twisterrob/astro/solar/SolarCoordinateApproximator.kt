...
internal class SolarCoordinateApproximator {
	...
	internal fun approximateSolarLongitude(jd: JulianDay): SolarCoordinates {
		...
		val D: Duration = jd - J2000_0
		// Mean anomaly of the Sun
		// Range: might not be 0-360.
		val g: Deg = (357.529.deg + (0.98560028 * D).rotation).canonicalMod(360.deg)
		// Mean longitude of the Sun
		// Range: might not be 0-360.
		val q: Deg = (280.459.deg + (0.98564736 * D).rotation).canonicalMod(360.deg)
		...
		val e: Deg = 23.439.deg - (0.00000036 * D).rotation
		...
		val EqT: Duration = q.time - RA.time
	...
	companion object {
		...
		internal val Deg.time: Duration
			get() = (this / DEG_PER_HOUR.deg).hours

		internal val Duration.rotation: Deg
			get() = this.toDouble(DAYS).deg
	}
}
